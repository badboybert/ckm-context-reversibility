...
CTXLAB <- c("liver"="liver","adipose-CR"="adipose\n(CR)","adipose-diet"="adipose\n(diet)","blood"="blood")
...
pa <- ggplot(cm, aes(ctx_a, ctx_b, fill=show)) +
  ...
  scale_x_discrete(labels=CTXLAB) + scale_y_discrete(labels=CTXLAB) +
  annotate("rect", xmin=1.5, xmax=3.5, ymin=1.5, ymax=3.5, fill=NA, color="black", linewidth=0.6) +
  labs(x=NULL, y=NULL, title="Reversibility clusters by tissue", subtitle="rev. effect correlates within adipose, not across tissues") +
...
pb <- ggplot(cc, aes(kind, concordance, color=kind)) +
  ...
  labs(x=NULL, y="directional concordance", title="Same tissue, different intervention agrees",
       subtitle="different tissues do not")
...
pc <- ggplot(long, aes(context, gene, fill=rev_beta)) +
  ...
  scale_x_discrete(labels=CTXLAB) +
  labs(x=NULL, y=NULL, title="The same molecule, different contexts",
       subtitle="the strongest reversers are confined to one tissue context") +
  theme(axis.text.x=element_text(size=9), axis.text.y=element_text(size=9, face="italic"),
        legend.key.width=unit(5,"pt"), legend.key.height=unit(9,"pt"),
        legend.title=element_text(size=9), legend.text=element_text(size=8))
...
pd <- ggplot(od, aes(kind, fold, color=kind)) +
  ...
  labs(x=NULL, y="reversible overlap (fold)", title="Shared within tissue",
       subtitle="reversible pole")
...
pe <- ggplot(oe, aes(kind, fold, color=kind)) +
  ...
  annotate("text", x=2, y=0.95, label="chance", size=2.0, color="grey35", fontface="italic") +
  scale_x_discrete(labels=c("within tissue"="within\ntissue","cross tissue"="cross\ntissue")) +
  labs(x=NULL, y="persistent overlap (fold)", title="Persistent ≈ chance",
       subtitle="no transfer either")
