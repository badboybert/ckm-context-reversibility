...
pa <- ggplot(pa_d, aes(lab, n_systemic_consensus, fill=fill)) +
  ...
  labs(x=NULL, y="cross-tissue consensus genes", title="Intervention acts through physiology",
       subtitle="surgery / diet systemic; exercise / drug local") +
...
CLAB <- c("BBS"="BBS (surgery)","DiRECT"="DiRECT (diet)","metformin"="metformin (drug)",
          "MS bariatric"="MS (surgery)","SGLT2i"="SGLT2i (drug)")
...
pb <- ggplot(pb_d, aes(rho, ylab)) +
  ...
  annotate("label", x=0.42, y=1.55, hjust=1, vjust=0.5, size=2.05, fill="white", label.size=0, lineheight=0.95, color="grey25",
           label="cross-platform +0.15 ≥\nsame +0.13\n(not a platform artifact)") +
  labs(x="Spearman vs semaglutide", y=NULL, title="Semaglutide behaves like weight loss",
       subtitle="correlates with surgery / diet, not SGLT2i") +
...
pc <- ggplot(pc_d, aes(intervention, marker, fill=rev_beta)) +
  ...
  labs(x=NULL, y=NULL, title="Layered plasma response",
       subtitle="IGF core + mechanism overlays") +
...
pdd <- ggplot(md, aes(label, median_abs_sig05, fill=label)) +
  ...
  annotate("label", x=1.5, y=0.97, size=2.05, fill="white", label.size=0, lineheight=0.95, color="grey20",
  ...
  labs(x=NULL, y="median |reversal| (sig genes)", title="Reversal as a clinical surrogate?",
       subtitle="hypothesis-generating (n=7–8/stratum)")
...
pe <- ggplot(pe_d, aes(a, b, fill=show)) +
  ...
  scale_x_discrete(labels=c("BBS (surg)"="BBS","DiRECT (diet)"="DiRECT","MS (surg)"="MS","sema"="sema","metformin"="metf","SGLT2i"="SGLT2i")) +
  scale_y_discrete(labels=c("BBS (surg)"="BBS","DiRECT (diet)"="DiRECT","MS (surg)"="MS","sema"="sema","metformin"="metf","SGLT2i"="SGLT2i")) +
  annotate("rect", xmin=0.5, xmax=4.5, ymin=2.5, ymax=6.5, fill=NA, color="black", linewidth=0.5) +
  labs(x=NULL, y=NULL, title="No consistent drug-class signature",
       subtitle="weight-loss group agrees; SGLT2i apart") +
...
# 2x3 grid (a,b,c / d,e,_): preserves a..e reading order = RESULTS callouts; short labels keep heatmaps legible.
fig <- wrap_plots(pa, pb, pc, pdd, pe, ncol=3) +
