...
pair[['kind','tissue','rho']].to_csv(os.path.join(OUT,'F2a_pair_correlations.csv'),index=False)
...
coh.to_csv(os.path.join(OUT,'F2b_cohesion.csv'),index=False)
...
pd.DataFrame(rows).to_csv(os.path.join(OUT,'F2c_pole_asymmetry.csv'),index=False)
...
core.to_csv(os.path.join(OUT,'F2d_universal_core.csv'),index=False)
