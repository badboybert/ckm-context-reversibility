...
pa <- ggplot(ga, aes(rho, comparison, fill=kind)) +
  ...
  labs(x="genome-wide Spearman ρ", y=NULL, title="No genome-wide transfer",
       subtitle="Black-vs-EUR ≈ 0 vs EUR-vs-EUR ceiling +0.30") +
...
pb <- ggplot(cb, aes(black_beta, gene)) +
  ...
  annotate("label", x=0.68, y=1.4, hjust=1, vjust=0, size=2.05, fill="white", label.size=0, lineheight=0.95,
           label=sprintf("directional concordance\n%.0f%% (p=0.009, n=%d)", sm$concordance[1]*100, sm$n[1])) +
  labs(x="Black-muscle reversal β", y=NULL, title="Biology-anchored transfer",
       subtitle="canonical muscle genes match EUR sign") +
  theme(axis.text.y=element_text(face="italic", size=8))

fig <- wrap_plots(pa, pb, ncol=2) +
  plot_annotation(tag_levels="a",
    title="Additional file 2 | Cross-ancestry transfer is directional-only, genome-wide underpowered (n=12)",
