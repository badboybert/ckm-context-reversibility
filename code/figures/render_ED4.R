...
pa <- ggplot(sa, aes(base_sd_mean, Z_responsiveness, color=class)) +
  ...
  annotate("label", x=0.78, y=1.15, hjust=1, vjust=1, size=2.1, fill="white", label.size=0, lineheight=0.95,
           label="persistent: base_sd 0.24, Z 0.15\nreversible: base_sd 0.27, Z 0.41\nbase_sd ratio 0.90 (median)") +
  labs(x="baseline SD", y="responsiveness Z", title="Persistent = low-variance floor",
       subtitle="lower variance AND lower responsiveness") +
...
pb <- ggplot(cb, aes(class, n, fill=layer)) +
  ...
  geom_text(aes(label=scales::comma(n)), position=position_dodge(0.75), vjust=-0.4, size=1.95) +
  ...
  labs(x=NULL, y="marks (log)", title="Variance-conditional classes",
       subtitle="persistent & reversible are minority poles") +
...
pc <- ggplot(fc, aes(cond, fold, group=grp, color=pole)) +
  ...
  annotate("text", x=2.0, y=0.85, label="chance", size=2.0, color="grey35", fontface="italic", hjust=1) +
  labs(x=NULL, y="cross-tissue overlap (fold)", title="Persistence collapses",
       subtitle="reversible holds; persistent → chance") +
...
fig <- wrap_plots(pa, pb, pc, ncol=3) +
  plot_annotation(tag_levels="a", title="Additional file 4 | Persistence is a variance-matched measurability floor, not a memory programme",
