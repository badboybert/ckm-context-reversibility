...
pa <- ggplot(dist, aes(R_restoration, layer, fill=layer)) +
  ...
  geom_text(data=summ, aes(x=median_R, y=layer, label=lab), vjust=-1.6, size=2.8, color=NEU) +
  scale_fill_manual(values=c(proteome=PRO, metabolome=METAB), guide="none") +
  scale_x_continuous(limits=c(-5,6), breaks=c(-4,-2,0,2,4,6)) +
  annotate("text", x=-4.6, y=2.5, label="toward lean →", hjust=0, size=2.8, fontface="italic", color="grey35") +
...
# near-chance contexts are marked.
...
pb <- ggplot(pb_d, aes(pct_toward_lean, lab, color=tissue)) +
  ...
  annotate("text", x=50, y=0.4, label="chance", size=2.8, color="grey35", fontface="italic", vjust=0) +
  ...
  annotate("text", x=69.6, y=6.9, label="open = not distinguishable\nfrom chance (P ≥ 0.05)",
           size=2.6, color="grey35", fontface="italic", hjust=1, vjust=0.5, lineheight=0.95) +
...
pc <- ggplot(pc_d, aes(rev2, rev5)) +
  ...
  annotate("label", x=-2.1, y=2.05, hjust=0, vjust=1, size=2.8, fill="white", linewidth=0, lineheight=0.95,
...
pe_d <- rd("F4d_yousri_points.csv"); lim2 <- 4   # panel d (proteome durability)   # heavy-tailed robust-z; out-of-range points dropped (not clamped) -> no boundary pile-up
pe <- ggplot(pe_d, aes(z2, z12)) +
  ...
  annotate("label", x=-3.9, y=3.8, hjust=0, vjust=1, size=2.8, fill="white", linewidth=0, lineheight=0.95,
...
pd_d <- rd("F4e_methyl_age_drift.csv")   # panel e (methylome caution)
# two INDEPENDENT cohorts at two intervals — no connecting line (it would imply a within-cohort time course).
pdd <- ggplot(pd_d, aes(interval_mo, spearman_delta_bmi)) +
  geom_point(size=2.6, color=METH) +
  geom_text(aes(label=cohort), vjust=-1.1, size=2.8) +
  scale_x_continuous(limits=c(0,24), breaks=c(3,18)) +
  scale_y_continuous(limits=c(0,0.7), breaks=c(0,0.2,0.4,0.6)) +
  annotate("text", x=12, y=0.13, label="two cohorts, two intervals —\nnot a time course", size=2.5, fontface="italic", color="grey35", lineheight=0.95) +
