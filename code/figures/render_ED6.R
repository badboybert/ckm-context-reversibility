...
# Reader-facing class labels (round 7: figure art used the internal ALL-CAPS class codes).
prettyclass <- function(x) {
  x <- gsub("within-DRUG","within drug",x); x <- gsub("within-SURGERY","within surgery",x)
  x <- gsub("DRUG×SURGERY","drug × surgery",x); x <- gsub("DRUG×DIET","drug × diet",x)
  x <- gsub("SURGERY×DIET","surgery × diet",x); gsub("weight-loss","weight loss",x)
}
...
pa <- ggplot(cm, aes(mean_fold, classpair, fill=hl)) +
  ...
  scale_y_discrete(labels=prettyclass) +
...
pp <- rd("ED6_pairs.csv"); pp$lab <- gsub("MS_bariatric","MS bariatric", paste(pp$a, pp$b, sep="×"))
...
pb <- ggplot(pp, aes(fold, lab, color=grp)) +
  ...
  scale_color_manual(values=c("within-DRUG"=UP, "cross-class (weight-loss)"=TXN,
                              "within-SURGERY"="grey55", "other"="grey75"), labels=prettyclass, name=NULL) +
